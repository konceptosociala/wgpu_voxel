...
#import rt/camera.wgsl as Camera
...
fn ray_color(ray: Ray::Ray, co: vec2<f32>, scan_depth: u32) -> vec3<f32> {
    var current_ray = ray;
    ...
    loop {
        ...
        if Chunk::hit(current_ray, 0.001, 3.40282347e+38, &hit_record) {
            // let direction = hit_record.normal + normalize(random_vec_in_unit_sphere(co), taa_config.jitter);
            // current_ray = Ray(hit_record.p, direction);
            // attenuation *= 0.5;
            // current_depth = current_depth - 1;
            // return vec3<f32>(1.0, 0.0, 0.0);
            return hit_record.normal * 0.5 + vec3<f32>(0.5);
        } else {
...
fn ray_velocity(current_ray: Ray::Ray, co: vec2<f32>) -> vec2<f32> {
    var hit_record = Ray::HitRecord();
...
fn cs_main(
    ...
    let camera = Camera::init(taa_config.canvas_width, taa_config.canvas_height, 10u, taa_config.jitter);

    var current_ray = Ray::on_coords(
        id.xy, 
        camera.first_pixel, 
    ...
    color_buffer[id.x + id.y * taa_config.canvas_width] = vec4<f32>(
        ray_color(
            current_ray, 
            vec2<f32>(f32(id.x), f32(id.y)), 
            camera.scan_depth, 
